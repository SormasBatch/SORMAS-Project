...
//			});
    ...
    public List<CaseExportDto> getExportList(
        ...
        if (!resultList.isEmpty()) {
            ...
            visitsCq.where(
                    ContactService.and(cb, caseRoot.get(AbstractDomainObject.ID).in(exportCaseIds), cb.isNotEmpty(visitsCqRoot.get(Contact.VISITS))));
            ...
            for (CaseExportDto exportCase : resultList) {
                ...
                if (lastCooperativeVisit != null) {
                    ...
                    if (lastCooperativeVisit.getSymptoms().getSymptomatic() == YesNoUnknown.YES) exportCase
                            .setLastCooperativeVisitSymptomatic(YesNoUnknown.YES);
                    else if (lastCooperativeVisit.getSymptoms().getSymptomatic() == YesNoUnknown.NO) exportCase
                            .setLastCooperativeVisitSymptomatic(YesNoUnknown.NO);
                    else exportCase.setLastCooperativeVisitSymptomatic(YesNoUnknown.UNKNOWN);
                }
            }
        ...
        if (sortProperties != null && !sortProperties.isEmpty()) {
            ...
            for (SortProperty sortProperty : sortProperties) {
                ...
                switch (sortProperty.propertyName) {
                    case CaseFollowUpDto.UUID:
                    case CaseFollowUpDto.REPORT_DATE:
                    case CaseFollowUpDto.FOLLOW_UP_UNTIL:
                        expression = caze.get(sortProperty.propertyName);
                        break;
                    case CaseFollowUpDto.PERSON:
            ...
            cq.orderBy(cb.desc(caze.get(Contact.CHANGE_DATE)));
        }
